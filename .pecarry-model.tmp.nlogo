...
to go
  ask turtles [
    ...
    ask future-patch [
     ; if (forest? = true) and (myself != nobody)
...
to generate-forest
  let xs (list)
  let ys (list)
  let num-forest ((percent-forest-cover / 100) * (count patches))
  ; Add initial coordinates to list
  repeat num-fragments [
    let x min-pxcor + random (max-pxcor - min-pxcor)
    let y min-pycor + random (max-pycor - min-pycor)
    ask patch x y [set forest? true]
    set xs lput x xs
    set ys lput y ys
  ]
  ; Add forest, until we reach desired forest cover
  while [count patches with [forest? = true] < num-forest]
    [
      let x (first xs)
      let y (first ys)
      let xdir (list 1 0 -1 0)
      let ydir (list 0 1 0 -1)
      (foreach xdir ydir
        [[a b] ->
          let xn (x + a)
          let yn (y + b)
          if valid-patch xn yn [
             ask patch xn yn [set forest? true]
             set xs lput xn xs
             set ys lput yn ys
          ]
         ] )
...
to-report valid-patch [x y]
  let ans true
  if x > max-pxcor or x < min-pxcor [report false]
  if y > max-pycor or y < min-pycor [report false]
  ask patch x y [if forest? = true [set ans false]]
  report ans
end
@#$#@#$#@
GRAPHICS-WINDOW
